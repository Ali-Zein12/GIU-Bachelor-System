                ...
                ELSE
                    BEGIN
                        ...
                        ELSE IF(@role LIKE 'student')
								BEGIN
								If( EXISTS( SELECT * FROM Student s INNER JOIN Bachelor_Project bp ON s.Assigned_Project_code = bp.Code WHERE s.s_id = @User_id))
									SELECT bp.* FROM Student s INNER JOIN Bachelor_Project bp ON s.Assigned_Project_code = bp.Code WHERE s.s_id = @User_id
								ELSE
									SELECT bp.* FROM Student s INNER JOIN MajorHasBachelorProject mb ON s.Major_code = mb.Major_code INNER JOIN Bachelor_Project bp ON bp.Code= mb.Project_code WHERE s.s_id = @User_id 
                ...
                BEGIN
					...
					BEGIN
						If( EXISTS( SELECT * FROM Student s INNER JOIN Bachelor_Project bp ON s.Assigned_Project_code = bp.Code WHERE s.s_id = @User_id))
							SELECT bp.* FROM Student s INNER JOIN Bachelor_Project bp ON s.Assigned_Project_code = bp.Code  WHERE s.s_id = @User_id
						ELSE					
							SELECT bp.*,a.* FROM Bachelor_Project bp INNER JOIN    MajorHasBachelorProject mb ON  mb.Project_code= bp.Code  
							INNER JOIN Academic a ON bp.Code = a.Academic_code INNER JOIN Student s ON s.Major_code = mb.Major_code WHERE s.s_id = @User_id 
							END
                    ELSE IF(@role LIKE 'company')
            ...
            ELSE IF(@ProjectType LIKE 'industrial')
                BEGIN
					...
					ELSE
                        ...
                        SELECT bp.*,i.* FROM Bachelor_Project bp INNER JOIN MajorHasBachelorProject mb ON  mb.Project_code= bp.Code  
                        INNER JOIN Industrial i ON bp.Code = i.Industrial_code  INNER JOIN Student s ON s.Major_code = mb.Major_code WHERE s.s_id = @User_id 
					END
					--
                    ELSE IF(@role LIKE 'company')
                        SELECT * FROM Bachelor_Project bp WHERE bp.Code IN(SELECT i.Industrial_code FROM Industrial i WHERE i.C_id = @User_id)
...
if(exists(select * from Student where @sid = s_id))
    begin
    UPDATE ProgressReport SET ComulativeProgressReportGrade = (SELECT AVG(Grade) FROM ProgressReport WHERE sid = @sid) WHERE sid = @sid
    IF(NOT EXISTS(SELECT * FROM ProgressReport WHERE sid = @sid))
        PRINT 'This student does not have any progress reports yet'
...
begin
    ...
    SET @grade3 = (select distinct ComulativeProgressReportGrade from ProgressReport where sid = @sid)
...
IF (@Company_id IS NULL OR @sid IS NULL OR @date IS NULL OR @Company_grade IS NULL)
    PRINT 'ONE OF THE INPUTS IS NULL'
ELSE IF exists( select * from Student S inner join Industrial I 
ON S.Assigned_Project_code= I.Industrial_code)
    BEGIN
        UPDATE ProgressReport SET  UpdatingUser_id = @Company_id,Grade = @Company_grade WHERE sid = @sid AND Date = @date
        IF EXISTS( SELECT * FROM GradeIndustrialPR P
        WHERE P.C_id =@Company_id AND P.sid =@sid AND P.Date = @date )
        BEGIN
                 UPDATE GradeIndustrialPR SET Company_grade=@Company_grade WHERE sid = @sid AND Date = @date
        END
        ELSE
        BEGIN
            INSERT INTO GradeIndustrialPR (sid, Date, C_id, Company_grade) VALUES (@sid, @date, @Company_id, @Company_grade)
   END
   END
ELSE
    PRINT 'Company does not have student with industrial bachelor project or wrong inputs'
GO
...
@Lecturer_id int, @start_time time, @end_time time, @date date, @meeting_point
varchar(5)
AS
            ...
            BEGIN
                ...
                ELSE 
                    BEGIN
                        ...
                        ELSE
                            SELECT m.*,MT.ToDoList FROM Meeting m LEFT OUTER JOIN MeetingToDoList MT ON m.Meeting_ID=MT.Meeting_ID WHERE m.L_id = @Lecturer_id and m.Meeting_ID = @meeting_id
    ...
    BEGIN
        ...
        ELSE
            IF(EXISTS(SELECT * FROM GradeAcademicThesis WHERE sid = @sid AND Title = @thesis_title))
                UPDATE GradeAcademicThesis SET L_id = @Lecturer_id, Lecturer_grade = @Lecturer_grade WHERE sid = @sid  -- if an external examiner already started grading the thesis
            ELSE
    ...
    BEGIN
        ...
        ELSE
            IF(EXISTS(SELECT * FROM GradeAcademicDefense WHERE sid = @sid AND Location = @defense_location))
                UPDATE GradeAcademicDefense SET L_id = @Lecturer_id, Lecturer_grade = @Lecturer_grade WHERE sid = @sid -- if an external examiner already started grading the defense
            ELSE
...
ELSE IF(NOT EXISTS(SELECT * FROM Academic WHERE Academic_code = @code))
    PRINT 'Lecturer can only grade progress reports belongling to academic bachelor projects'
ELSE
    BEGIN
      ...
      END
      ELSE    
            UPDATE GradeAcademicPR SET L_id = @Lecturer_id, Lecturer_grade = @lecturer_grade WHERE sid = @sid
        UPDATE ProgressReport SET UpdatingUser_id = @Lecturer_id,Grade = @lecturer_grade WHERE sid = @sid AND Date = @date
...
AS
IF exists(SELECT * FROM GradeAcademicThesis a WHERE a.sid=@sid AND a.Title =@thesis_title)
    UPDATE  GradeAcademicThesis SET EE_grade=@EE_grade, EE_id = @EE_id WHERE sid=@sid AND Title =@thesis_title
ELSE
	INSERT INTO GradeAcademicThesis (EE_id, sid, Title, EE_grade) VALUES (@EE_id, @sid, @thesis_title, @EE_grade);
GO
...
AS
IF exists(SELECT * FROM GradeAcademicDefense AD
...
ELSE
	INSERT INTO GradeAcademicDefense (EE_id, sid, Location, EE_grade) VALUES (@EE_id, @sid, @defense_location, @EE_grade)
GO
  ...
  BEGIN
    ...
    ELSE
        BEGIN
			...
			IF  @User_type LIKE 'company' or @User_type like 'companies'
                SELECT * FROM Users INNER JOIN  Company c ON Users.user_id= c.Company_id
        END
...
ELSE
    BEGIN
        ...
        SET @t= (select Role from Users where @User_id= user_id)
        if @t <>@User_Type
         print'invalid role'
         else EXEC ViewProfile @User_id
    END
    END
GO
...
UPDATE Academic SET TA_id = @TA_id WHERE Academic_code = @proj_code 
GO
    ...
    BEGIN
        ...
        ELSE
            IF(EXISTS(SELECT * FROM GradeIndustrialDefense WHERE sid = @sid AND Location = @defense_location))
               ...
               UPDATE GradeIndustrialDefense SET E_id = @Employee_id, Employee_grade = @Employee_grade where sid = @sid and 
                Location = @defense_location-- if an external examiner already started grading the defense
            ELSE        
...
Employee E ON E.Staff_id = I.E_id WHERE I.E_id = @Employee_id AND S.s_id = @sid)
...
BEGIN
    ...
    FROM Employee
    WHERE Email = @email AND EmployeePassword = @password
